...
version: 0.0.15
*/
import { addClass as addClass$1, removeClass as removeClass$1, hasClass as hasClass$1, isObject, now, isUndefined, addEvent } from '@daybrush/utils';
...
function getKeyframesStructure(propertiesInfo, maxTime) {
  ...
  if (length >= 2) {
    var startFrame = length !== 2 && frames[0][0] === 0 && frames[0][1] === 0 && isUndefined(frames[0][2]) && !properties.length ? frames[1] : frames[0];
  ...
  frames.forEach(function (_a, i) {
    ...
    if (frames[i + 1]) {
      ...
      if (!isItScene && !isUndefined(value) && !isUndefined(nextValue) && valueText !== nextValueText) {
        keyframeLines.push({
